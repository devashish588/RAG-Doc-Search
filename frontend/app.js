...
function renderConfidence(conf) {
  if (!conf) { confidenceBox.innerHTML = '<div class="muted">â€”</div>'; return; }
  var level = conf.level || 'unknown';
  var color = level === 'high' ? 'ok' : level === 'medium' ? '' : 'fail';
  var score = typeof conf.overall_score === 'number' ? (conf.overall_score * 100).toFixed(0) + '%' : 'â€”';
  ...
  var html =
    ...
    '</div>' +
    '<div class="muted">' +
      'Retrieval: ' + (typeof conf.retrieval_confidence === 'number' ? (conf.retrieval_confidence * 100).toFixed(0) + '%' : 'â€”') +
      ' · Grounding: ' + (typeof conf.grounding_confidence === 'number' ? (conf.grounding_confidence * 100).toFixed(0) + '%' : 'â€”') +
    '</div>';
...
function renderTrace(trace) {
  if (!trace) { traceBox.innerHTML = '<div class="muted">â€”</div>'; return; }
  var html = '';
  ...
  if (!html) html = '<div class="muted">â€”</div>';
